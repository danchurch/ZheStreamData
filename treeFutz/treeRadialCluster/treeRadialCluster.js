...
function Tree(data, { // data is either tabular (array of objects) or hierarchy (nested objects)
  ...
  const svg = d3.select("#zoop").append('svg')
  ...
  svg.append("g")
      ...
      //      .angle(d => d.x)
      //      .radius(d => d.y));
      // but d3.linkRadial has a bezier bump built into it, try something else?
      .selectAll( "line" ).data( root.links() ).enter()
...
d3.text("grahamAllSeqsTreeRelabeled.nwk").then(function(nwk){
    let grahamJSON = parseNewick(nwk);
    console.log( grahamJSON );

    chart = Tree(grahamJSON, {
      tree: d3.cluster,
      label: d => d.name,
      children: d => d.branchset,
      width: 1152,
      height: 1152,
      margin: 100
    })
...
// someone was in a similar position as myself, concerning straightening the links..

// https://stackoverflow.com/questions/59322327/d3-tree-diagram-with-straight-links
